...
class BoardImages:
    def __init__(self, image: np.ndarray):
        ...
        UpdatedImageStuff.remove_user_interface(image = self.board_image, replacement_color = ColorEnums.CommonGameColors.BACKGROUND.value)
        ...
        if self.get_adjacency_type():
            ...
            UpdatedImageStuff.remove_adjacency_symbol(image = self.board_image, replacement_color = ColorEnums.CommonGameColors.BACKGROUND.value)
        else:
    ...
    def create_binary_image(self) -> None:
        ...
        UpdatedImageStuff.remove_mine_counts(image = self.binary_image, replacement_color = ColorEnums.CommonGameColors.BACKGROUND.value)
    ...
    def create_point_image(self) -> None:
        ...
        self.point_image.convert_type(ProjectEnums.ImageType.RGB)
        ...
        self.point_image.image[diff_in_displace > 0.005 * diff_in_displace.max()] = ColorEnums.CommonProgramColors.RGBColors.VERTEX.value
        ...
        for y in range(self.binary_image.height):
            for x in range(self.binary_image.width):
                if (self.binary_image.pixel_is_color(y = y, x = x, check_color = ColorEnums.CommonProgramColors.GrayscaleColors.TILE.value) and
                        self.point_image.pixel_is_color(y = y, x = x, check_color = ColorEnums.CommonProgramColors.RGBColors.VERTEX.value)):
                    self.point_image.set_pixel(y = y, x = x, fill_color = ColorEnums.CommonProgramColors.RGBColors.TILE.value)
