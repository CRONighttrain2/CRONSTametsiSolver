...
from ProjectEnumsV2 import ImageType
...
class ImageData:
    ...
    def to_binary_image(self, white_color):
        """
        creates a new image data then turns its image into a binary image of the original where pixels that are white_color
        are white and pixels that aren't are black then returns the new image
        :param white_color: color to be set to white in the image
        """
        ...
        for y in range(self.height):
            for x in range(self.width):
                if self.pixel_is_color(y = y, x = x, check_color = white_color):
                    new_image.set_pixel(y = y, x = x, fill_color = ProjectEnumsV2.ColorEnums.CommonProgramColors.GrayscaleColors.BACKGROUND.value)
                else:
                    new_image.set_pixel(y = y, x = x, fill_color = ProjectEnumsV2.ColorEnums.CommonProgramColors.GrayscaleColors.TILE.value)
        return new_image

    def convert_type(self, new_image_type: ImageType) -> None:
        """
        converts current image into a new type
        :param new_image_type: new image type
        """
    ...
    def get_pixel(self, y: int, x:int, warped: bool = False):
        """
        gets the color of a pixel

        :param warped: (optional), default: False, when True: scales the y and x relative to the runners screen size
        """
        if warped:
            y = self.warp_y(y = y)
            x = self.warp_x(x = x)
    ...
    def pixel_is_color(self, y: int, x: int, check_color, warped: bool = False) -> bool:
        """
    ...
    def set_area_color(self, y1: int, x1: int, y2: int, x2: int, fill_color: Any, warped: bool = False) -> None:
        """
        ...
        :param warped: (optional), default: False, when True: scales the area relative to the runners screen size
    ...
    def set_pixels_in_area_if_color(self, y1: int, x1: int, y2: int, x2: int, fill_color, check_color, warped: bool = False) -> None:
        """
